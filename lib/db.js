...
export async function recordReview({ cardId, userId, rating, idempotencyKey, solvedFromScratch } = {}) {
  ...
  const eventRows = await db.query(
    `WITH seeded_parameter AS (
     ...
     event_claim AS (
       INSERT INTO fsrs_review_events
         (id, owner_id, card_id, rating, solved, occurred_at, scheduled_at,
          algorithm_version, parameter_version, idempotency_key)
       SELECT $3, $4, $5, $6, $7, $8, $9, $10, parameter_source.version, $11
       FROM (
     ...
     card_update AS (
       UPDATE cards c
       SET easiness_factor = $12, interval = $13, repetitions = $14,
           next_review = $15, last_review = $16, last_quality = $17, updated_at = $8
       FROM event_claim e
       WHERE c.id = $5 AND c.owner_id = $4
         AND c.updated_at = $18::timestamptz
     ...
     schedule_write AS (
       ...
       SELECT c.owner_id, c.id, $9, 0, 0, $19, $1, $8
       FROM card_update c
     ...
     practice_write AS (
       ...
       SELECT c.owner_id, c.id, 'active', $8, $20, $8
       FROM card_update c
       WHERE $21::boolean
       ON CONFLICT (owner_id, card_id) DO UPDATE SET
     ...
     SELECT e.id, e.card_id, e.rating, e.solved, e.occurred_at, e.scheduled_at,
            e.algorithm_version, e.parameter_version, e.idempotency_key
     FROM event_claim e
     CROSS JOIN claim_guard`,
    [
      1,
      JSON.stringify({ fsrs: 'inactive', mode: 'compatibility-shadow', activeScheduler: false }),
      crypto.randomUUID(), owner, id, normalizedRating, solvedFromScratch, now, scheduled.dueAt, 1, key,
      ...
      preRead.updated_at, normalizedRating === 'again' ? 'relearning' : 'review',
      practiceScheduled?.dueAt ?? null, solvedFromScratch,
    ]
  );
