...
import { normalizeRating, scheduleNextDue } from './fsrs.js';
...
export async function recordReview({ cardId, userId, rating, idempotencyKey, solvedFromScratch } = {}) {
  ...
  const existingRows = await db.query(
    `SELECT id, card_id, rating, solved, occurred_at, scheduled_at,
            algorithm_version, parameter_version, idempotency_key
  ...
  // UTC is the fallback only when the owner has no preference row.
  const timeZone = preferenceRows.length ? preferenceRows[0].timezone : 'UTC';
  const now = new Date();
  ...
  await db.query(
    ...
    [1, JSON.stringify({ fsrs: 'inactive', mode: 'compatibility-shadow', activeScheduler: false })]
  );

  const legacyQuality = RATING_TO_LEGACY_QUALITY[normalizedRating];
  const compatibilitySm2 = sm2Calc(legacyQuality, card.sm2 || {});
  // Legacy SM-2 schedules Again for today; the shadow boundary must not.
  if (normalizedRating === 'again') {
    compatibilitySm2.nextReview = scheduled.dueAt.slice(0, 10);
  }
  const legacyRows = await db.query(
    `UPDATE cards
     SET easiness_factor = $3, interval = $4, repetitions = $5,
         next_review = $6, last_review = $7, last_quality = $8, updated_at = $9
     WHERE id = $1 AND owner_id = $2
     RETURNING id`,
    [id, owner, compatibilitySm2.easinessFactor, compatibilitySm2.interval,
      compatibilitySm2.repetitions, compatibilitySm2.nextReview, compatibilitySm2.lastReview,
      compatibilitySm2.lastQuality, now]
  );
  if (!legacyRows.length) return { missing: true };

  await db.query(
    `INSERT INTO fsrs_card_schedules
       (owner_id, card_id, due_at, stability, difficulty, state, schedule_version, updated_at)
     ...
     ON CONFLICT (owner_id, card_id) DO UPDATE SET
       ...
       schedule_version = EXCLUDED.schedule_version, updated_at = EXCLUDED.updated_at`,
    [owner, id, scheduled.dueAt, 0, 0, normalizedRating === 'again' ? 'relearning' : 'review', 1, now]
  );
  if (solvedFromScratch) {
    const practiceScheduled = scheduleNextDue({
      mode: 'practice',
      rating: normalizedRating,
      timeZone,
      now,
    ...
    await db.query(
      ...
      [owner, id, now, practiceScheduled.dueAt]
    );
  }
  ...
  const eventRows = await db.query(
    `INSERT INTO fsrs_review_events
     ...
     RETURNING id, card_id, rating, solved, occurred_at, scheduled_at,
               algorithm_version, parameter_version, idempotency_key`,
    [crypto.randomUUID(), owner, id, normalizedRating, solvedFromScratch, now, scheduled.dueAt, 1, 1, key]
  );
