...
/** Insert or update a card only when its existing row belongs to ownerId. */
...
export async function replaceCardsForOwner(cards, ownerId) {
  ...
  for (const card of validated) await upsertCard(card, owner);
...
/** Claim only currently unowned legacy cards and designs for a supplied user. */
export async function claimLegacyContent(user) {
  if (!user || typeof user !== 'object') throw new Error('user is required');
  const clerkId = requireId(user.clerkId, 'user.clerkId');
  await upsertUser({ clerkId, email: user.email, displayName: user.displayName });
  const db = requireDatabase('claim legacy content');
  const claimedCards = await db.query(
    'UPDATE cards SET owner_id = $1 WHERE owner_id IS NULL RETURNING id',
    [clerkId]
  );
  const claimedDesigns = await db.query(
    'UPDATE designs SET owner_id = $1 WHERE owner_id IS NULL RETURNING id',
    [clerkId]
  );
  return { cards: claimedCards.length, designs: claimedDesigns.length };
}
