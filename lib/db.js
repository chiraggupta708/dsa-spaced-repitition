...
export async function save(data) {
  ...
  //    NOTE: use `<> ALL($1)` with the array passed as a real parameter.
  //    The tagged-template form `id NOT IN (${ids})` serializes the JS array
  //    as a string literal ('{id1,id2}'), which makes `id NOT IN ('...')`
  //    TRUE for EVERY row -> a silent mass-delete via cascade. <> ALL($1) is
  //    correct Postgres text-array semantics.
  if (cards.length > 0) {
    const ids = cards.map((c) => c.id);
    await sql.query('DELETE FROM cards WHERE id <> ALL($1)', [ids]);
  }
...
function rowToDesign(row) {
  const tags = Array.isArray(row.tags) ? row.tags.map((t) => t.name || t) : [];
  const d = { tags };
  DESIGN_COLS.forEach((c) => { d[c] = row[c]; });
  return d;
}

/**
 * Load designs. opts.kind ('lld'|'hld') filters; opts.tag filters by tag name.
 */
export async function loadDesigns(opts) {
  ...
  const rows = await sql`
    ...
    ${opts.kind ? sql`WHERE d.kind = ${opts.kind}` : sql``}
    GROUP BY d.id
  ...
  if (opts.tag) {
    ...
    designs = designs.filter((d) =>
      (d.tags || []).some((t) => String(t).toLowerCase() === tag)
    );
  }
  return { designs };
}
...
/**
 * Save a single design (insert or update by id). Tags are upserted by name
 * and this design's tag links are replaced per-design (no global wipe).
 */
export async function saveDesign(design) {
  if (!sql) throw new Error('DATABASE_URL not set — cannot save');
  const id = design.id || generateId();
  const kind = design.kind === 'hld' ? 'hld' : 'lld';
  const now = new Date();
  const createdAt = design.created ? new Date(design.created) : now;
  const updatedAt = now;
  ...
  await sql.query(
    `INSERT INTO designs
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       my_approach = EXCLUDED.my_approach,
       canonical_approach = EXCLUDED.canonical_approach,
       components = EXCLUDED.components,
       relationships = EXCLUDED.relationships,
       patterns = EXCLUDED.patterns,
       api = EXCLUDED.api,
       estimations = EXCLUDED.estimations,
       tradeoffs = EXCLUDED.tradeoffs,
       notes = EXCLUDED.notes,
       updated_at = EXCLUDED.updated_at`,
    [
      id, kind, design.title || '', design.requirements || '',
      design.my_approach || '', design.canonical_approach || '',
      design.components || '', design.relationships || '',
      design.patterns || '', design.api || '', design.estimations || '',
      design.tradeoffs || '', design.notes || '', createdAt, updatedAt,
    ]
  );

  // Replace this design's tag links.
  await sql.query('DELETE FROM designs_tags WHERE design_id = $1', [id]);
  ...
  for (const tagName of tags) {
    ...
    const tagId = tagResult[0].id;
    await sql.query(
      'INSERT INTO designs_tags (design_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
      [id, tagId]
    );
  }
...
export async function deleteDesign(id) {
  if (!sql) throw new Error('DATABASE_URL not set — cannot delete');
  await sql.query('DELETE FROM designs WHERE id = $1', [id]);
  return { ok: true };
}
