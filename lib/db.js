...
export async function getPracticeSummary(ownerId) {
  ...
  try {
    ...
    error.practiceStage = 'summary_aggregate';
    throw error;
  }
  let duePage;
  try {
    duePage = await listPracticeQueue(owner, { bucket: 'due', limit: 1 });
  } catch (error) {
    error.practiceStage = 'summary_due_queue';
    throw error;
  }
  let nextPage = duePage;
  if (!duePage.items.length) {
    try {
      nextPage = await listPracticeQueue(owner, { bucket: 'first-check', limit: 1 });
    } catch (error) {
      error.practiceStage = 'summary_first_check_queue';
      throw error;
    }
  }
...
export async function listPracticeQueue(ownerId, options = {}) {
  ...
  const snapshotAt = cursor?.snapshotAt || serverNow.toISOString();
  const params = [owner, snapshotAt, snapshotAt];
  const where = [
    'c.owner_id = $1',
    'COALESCE(s.updated_at, c.updated_at) <= $3',
  ];
  if (bucket === 'due') {
