...
export async function getPracticeSummary(ownerId) {
  ...
  const rows = await sql.query(
    `SELECT
       COUNT(*) FILTER (WHERE s.next_practice_at IS NOT NULL AND s.next_practice_at <= $2) AS due_count,
       COUNT(*) FILTER (WHERE s.last_independent_solve_at IS NULL) AS first_check_count,
       COUNT(*) FILTER (WHERE ${recognition.suggested}) AS recognition_trap_count,
       COALESCE(MAX(s.revision), 0) AS version
     FROM cards c
