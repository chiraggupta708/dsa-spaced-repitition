...
export function normalizeLldAiInput(input) {
  ...
  return {
    ...
    review: source.review === undefined ? null : normalizeLldReview(source.review),
  };
}
...
export function createFallbackLldAiResponse(input) {
  ...
  const missingPoints = expectation.terms
    .filter((term) => !lower.includes(term))
    .slice(0, 3)
    .map((term) => `Make the ${term} decision explicit.`);
