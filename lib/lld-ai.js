...
export function normalizeLldAiProviderPayload(input) {
  const source = record(input, 'aiProviderResponse');
  return { ...source, assessment: normalizeProviderAssessment(source.assessment) };
}
...
function resolveLldAiProvider(env = process.env) {
  ...
  return {
    ...
    model: env.LLD_AI_MODEL || (isOpenRouter ? 'openai/gpt-4o-mini' : 'gpt-4o-mini'),
        ...
        messages: [
          {
            ...
            content: [
              ...
              'Return JSON only with assessment exactly one of missed, partial, or clear; include feedbackMd, missingPoints, topImprovements, nextDrill, followUpQuestion, hintMd, shouldReveal.',
            ].join(' '),
          },
